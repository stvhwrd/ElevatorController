...
MovingUp:
	...
	bl		WaitAndPoll
	add		r8,r8,#1
	str		r8,[r3]		@floor incremented by one
	cmp		r8,r6
	beq		ReachedHighDestination
	blt		MovingUp
@	mov		r5,#0	@ R5=1 to indicate up direction
@	BL		UpdateDirectionScreen	@UpdateDirectionScreen(R5:direction)
@	mov		r5,#1			@point (stopped)
@	BL		UpdateFloor		@UpdateFloor(R3:&floor;R5:moving)
@	mov		r0, #NO_LED		@Left LED on
@	swi		SWI_SETLED
@	ldr		r7,[r3]
@	cmp		r7,#4
@	beq 	ReachedHighDestination
...
MovingDown:
	...
	bl		WaitAndPoll
	sub		r8,r8,#1
	str		r8,[r3]		@floor decremented by one
	cmp		r8,r6
	beq		ReachedLowDestination
	bgt		MovingDown
@	mov		r5,#	@ R5=0 to indicate stopped
@	BL		UpdateDirectionScreen	@UpdateDirectionScreen(R5:direction)
@	mov		r5,#1			@point (stopped)
@	BL		UpdateFloor		@UpdateFloor(R3:&floor;R5:moving)
@	mov		r0, #NO_LED		@Left LED on
...
OpenDoors:
	STMFD	sp!,{r1-r10,lr}
	mov		r5,#2			@ Display Idle State on all outputs
	BL		UpdateDirectionScreen	@UpdateDirectionScreen(R5:direction)
	mov		r5,#1			@ point (stopped)
	BL		UpdateFloor		@UpdateFloor(R3:&floor;R5:stopped)
	mov		r0, #NO_LED		@LEDs off
